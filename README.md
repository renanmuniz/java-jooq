...
jOOQ's code generator introspects a **live database** — it does not read `.sql` files directly. This means the `users`
table must exist in PostgreSQL **before** you run `./mvnw clean generate-sources`. The `schema.sql` file is the
single source of truth for the table definition and is used to initialise the database the first time the container starts.

### How to reference it in `docker-compose.yml`

Docker's official PostgreSQL image executes any `.sql` file mounted under `/docker-entrypoint-initdb.d/` **once**, on
first container start (i.e. when the data volume is empty). Wire it up with a `volumes` bind-mount:
